...
const Client = () => {
    ...
    useEffect(() => {
        const fetchBranchData = async () => {
            if (user) {
                const clientData = await ApiAxiosGet('client', user.newAccessToken);
    ...
    const handleDelete = async (deleteId: String) => {
        ...
        try {
            ...
            if (response && response.acknowledged === true) {
                setIsModalOpen(false);
                setBranchTitle(null);
                navigate('/users', { state: { success: true, message: 'client Removed successfully!' } });
        ...
        <div className="bg-white flex flex-col w-full h-full gap-4 p-4 rounded">
            ...
            <div className="flex flex-col gap-4 ">
                ...
                <div>
                    ...
                    <div className="overflow-x-auto grid md:grid-cols-2 gap-4">
                        {
                                ...
                                clientData.map((client) => (
                                    <>
                                        ...
                                        <div className="card-body">
                                            <h2 className="card-title">{client.title}</h2>
                                            <p>{client.address}</p>
                                            <p>{client.country}</p>
                                            <div className="">
                                                <p className=" font-semibold">Client Manager : </p>
                                            ...
                                            <div className="card-actions justify-end">
                                                <Link to={`/project`} className="btn btn-primary">View Project</Link>
                                            </div>
                                        </div>
                                    </div>
                                    </>
